...
- Which decisions affect `<Product X>`?
...
Граф знаний **плотный** — это отражение реальной связности бизнеса (метрики↔синтезы↔продукты↔решения, продуктовые домены↔финансы). Доказано (3-way A/B): резать связи **нельзя** — это вредит извлекаемости. Поэтому плотный граф не читают целиком; в него входят **точечно через линзы**, фильтруя отображение, а не данные. Требуется плагин Dataview.
...
- **Groups (раскраска по доменам)** настроены в `.obsidian/graph.json` — продукты, метрики, синтезы, стратегия, решения видны разными цветами; кластеры-домены различимы даже в плотном графе.
