...
`claim::` frontmatter and `entity::` sub-document nodes were part of an earlier version of the method. Isolated A/B experiments (90 solver-agents each, blind judge, fixed golden answers) retired both. The honest reading: their *overall* mean accuracy effect was within noise (95% CI includes 0 at n=3/question) — they are **neutral on average**, not uniformly worse. What is statistically solid is **local and large**: where an entity sub-page fixed a number that competed with the canonical page, it acted as an *attractor* — pulling solvers onto its value and cutting off the broader search — dropping accuracy 0.95 → 0.48 and recovering to 0.97 on removal (pooled +0.48, 95% CI [+0.42, +0.54], reproduced across two runs, mechanism traced by hand). A separate powered study (300 solvers, bootstrap CI, placebo control) found that *adding* provenance edges has no effect either — the core is on a plateau. Rule that survives: numbers live in exactly one place (the metric or source page), reached via typed `derived_from::`/`evidence::` edges — never duplicated into a sub-document fact-node.
